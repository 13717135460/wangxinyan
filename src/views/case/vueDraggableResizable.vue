<template>
	<div class="case-box draggable-resizable">
		<div class="controls-list">
			<div :class="commonClassName" @click="addControl(1)" class="controls-item controls-text">
				<p>文本</p>
			...
			<div :class="commonClassName" class="controls-item controls-date" @click="addControl(2)">签章</div>
			<div :class="commonClassName" class="controls-item controls-select" @click="addControl(3)">签名</div>
			<div :class="commonClassName" class="controls-item controls-sign" @click="addControl(4)">选项</div>
			<div :class="commonClassName" class="controls-item controls-seal" @click="addControl(5)">日期</div>
		</div>
		<div class="drag-wrap">
			<vue-draggable-resizable
			...
			<div :class="commonClassName" class="inner-container">
					{{ filterSignatory(item.signatory) }}<span class="name">({{ item.name }})</span>
			</div>
			<i class="close el-icon-error" @click="removeControl(item.customId)"></i>
		...
		<div class="set-wrap">
			<div class="control-set-item">
				...
				<el-input size="mini" v-model="textVal" @change="textChange"></el-input>
			</div>
				<div class="control-set-item" v-if="['text', 'date', 'select'].includes(activedType)">
					<p class="title">字体设置</p>
					<el-select size="mini" v-model="fontSizeValue" @change="selectFontsize" placeholder="请选择">
						<el-option
...
export default {
  components: {
	...
	methods: {
		// 计算距离
		getDomData() {
			let domCtrList = document.getElementsByClassName('controls-list')[0]
...
.drag-wrap {
	...
	.lq-draggable-date,
	.lq-draggable-sign,
	.lq-draggable-select { border: dashed 1px #000; }
  .lq-draggable-seal {
		.seal-inner {
			width: 100%;
			height: 100%;
